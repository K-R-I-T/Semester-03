...
public class BSTree {
    ...
    */
    ...
    Node insert(Node p, Bird x) {
        if (p == null) {
            return new Node(x);
        }
        if (x.price < p.getInfo().price) {
            p.left = insert(p.left, x);
        } else if (x.price > p.getInfo().price) {
    ...
    void f4() throws Exception {
        ...
        f.writeBytes("\r\n");
        /**
         * Question 2.4: Reset the Bird's color of all leaf Nodes to 100
